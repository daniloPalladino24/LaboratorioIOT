...
INFLUXDB_TOKEN = "TOKEN_INFLUX_DB"  # Sostituisci con il tuo token
INFLUXDB_ORG = "microbit-org"      # Sostituisci con la tua organizzazione
INFLUXDB_BUCKET = "BUCKET_NAME"  # Sostituisci con il tuo bucket

# Configurazione Seriale
SERIAL_PORT = "SERIALE"  # Porta seriale per la micro:bit
BAUD_RATE = 115200
...
def create_influxdb_points(data, timestamp=None):
    """Crea punti dati essenziali per InfluxDB per sistema servo controller"""
    if timestamp is None:
        timestamp = datetime.now(timezone.utc)
    
    # Crea un singolo punto con solo i dati essenziali
    point = Point("microbit_data") \
    ...
    return [point]  # Ritorna lista con un solo punto
